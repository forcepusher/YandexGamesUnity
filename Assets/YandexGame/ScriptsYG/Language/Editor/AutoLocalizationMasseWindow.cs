...
namespace YG.Insides
{
    public class AutoLocalizationMasse : EditorWindow
    {
        ...
        private void OnGUI()
        {
            if (GameObject.FindObjectOfType<YandexGame>(true).infoYG.translateMethod == InfoYG.TranslateMethod.AutoLocalization)
            {
                GUILayout.Space(10);
                ...
                if (GUILayout.Button("Search for all objects on the scene by type TEXT (Legasy)", GUILayout.Height(30)))
                {
                    ...
                    foreach (Text obj in SceneAsset.FindObjectsOfType<Text>(true))
                    {
                ...
                if (GUILayout.Button("Search for all objects on the scene by type LANGUAGE_YG", GUILayout.Height(30)))
                {
                    ...
                    foreach (LanguageYG obj in SceneAsset.FindObjectsOfType<LanguageYG>(true))
                    {
                        objectsTranlate.Add(obj.gameObject);
                    }
                }
